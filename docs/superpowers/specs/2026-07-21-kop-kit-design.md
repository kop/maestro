...
All agents except `general-purpose` are read-only: `Glob, Grep, Read` plus the minimum extras named below. `Bash` on the review agents exists only to fetch the diff (`git diff`, `gh pr diff`) for standalone use — none of them run anything else. `code-reviewer` and `security-reviewer` also get `Agent(Explore, peer)` so they can spawn `Explore` and `peer` as nested subagents and nothing else, and both run at `effort: high`. The parenthesized type-list is only enforced when an agent runs as a `--agent` main thread — for the usual dispatched-subagent path Claude Code ignores it, so each reviewer's prompt also states the two-type restriction. The `maestro` orchestrator gets `Agent`, `Skill`, and `TodoWrite` (no `Bash`) so it can only delegate.
