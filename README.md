...
- `stacked-prs` — how to split a chain of dependent changes into a stack of PRs (GitHub `gh stack` extension plus the git-native fallback), and when to prefer stacking over one large PR. Triggers on stacked-PR requests or naturally-stacked multi-PR work.
